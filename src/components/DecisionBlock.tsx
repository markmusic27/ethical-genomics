...
const DecisionBlock = ({ block, opt }: { block: Block; opt: number }) => {
  return (
    <div
      ...
      style={{
        backgroundImage:
          "linear-gradient(to bottom, rgba(0,0,0,0.3) 10%, rgba(0,0,0,0) 40%, rgba(0,0,0,0) 60%, rgba(0,0,0,0.6) 90%), url('/game/0.png')",
      }}
    >
      <div className="flex h-full w-full flex-col justify-between">
        <Text className="text-[14px] text-white">{"Option " + (opt + 1)}</Text>
